...
def detect_objects(image,model_op,conf_th):
    global input_width, input_height
    with open("assets/labelmap.pbtxt", 'r') as f:
      labels = [line.strip() for line in f.readlines()]
    if model_op == "SSD":
      interpreter = tf.lite.Interpreter(model_path="models/ssd/SSD_detect.tflite")
    elif model_op == "Eff":
      ...
      for nb in range(int((tpdata2['TIME'].max() - tpdata2['TIME'].min()) / s_interval)):
        ...
        if pt2 < tpdata2['TIME'].max():
            ...
            if fpngname is not None:
              ...
              detection, input_width, input_height = detect_objects(image,model_op,conf_th)
    ...
    if detection2 != 505:
      ...
      if not to_best.empty:
        ...
        for fpname in best_df['filename']:
          ...
          detection2, input_width, input_height = detect_objects(image2,model_op,conf_th)
