...
def detect_objects(image):
    ...
    if model_op == "SSD":
      interpreter = tf.lite.Interpreter(model_path="ssd/detect.tflite")
    elif model_op == "Eff":
...
def get_fits_path(target_id, selected_obs_id, period_input):
    global nb
...
def above_conf_detection(target_id, kis, period_input):
  ...
  for indx in kis:
    sy = 1
    detection2, conf_to_best = get_fits_path(target_id, indx, period_input)
    if detection2 != 505:
      ...
      if not to_best.empty:
        ...
        for fpname in best_df['filename']:
          ...
          detection2 = detect_objects(image2)
          ...
          if not dfdet2.empty: 
            ...
            result0 = detection2, dfdet3, encoded_image, fpname
...
def results():
    ...
    global user_input_id, period_input, period_factor, conf_th, model_op
    ...
    if 'user_input' in request.form:
      ...
      for litem in result11:
        detec, dfdet3, encoded_image, fpname = litem
        for itek in detec:
...
########################################## IF file upload
        ...
        for index, row in filedf.iterrows():
          ...
          model_op = request.form['radio_option']
          kis0 = get_observation_ids(user_input_id)
          print(user_input_id,"=========",kis0, type(kis0),"===size", kis0.size)
          if kis0.size > 0:         
            kis = sorted(kis0)     
            detco = []
            filnm = []
...
def downl():
    try:
        ...
        #text_response = make_response(send_file(fnm, as_attachment=True))
        #text_response.headers["Content-Disposition"] = f"attachment; filename={fnm}"
